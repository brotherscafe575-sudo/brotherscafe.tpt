...
from .models import ShopSettings, Table, Category, MenuItem, Order, OrderItem, Discount, CustomerProfile, Combo, ComboItem, OfferBanner, CartOffer
import openpyxl
...
def export_orders_excel(modeladmin, request, queryset):
    ...
    headers = ['Order #', 'Date', 'Customer', 'Phone', 'Table', 'Items', 
               'Items', 'Subtotal', 'Discount%', 'Discount Amt', 'Total', 'Status', 'Payment', 'Cash Received', 'Change Due']
    ws.append(headers)
    for order in queryset:
        ...
        ws.append([
            ...
            float(order.cash_received or 0),
            float(order.change_amount or 0),
        ])
    buf = BytesIO()
    wb.save(buf)
    buf.seek(0)
...
class OrderAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['order_number', 'subtotal', 'discount_amount', 'total_amount', 'cash_received', 'change_amount', 'created_at']
    ...
    fieldsets = (
        ...
        ('Billing', {'fields': ('subtotal', 'discount_percent', 'discount_amount', 'total_amount', 'cash_received', 'change_amount')}),
...
class OfferBannerAdmin(admin.ModelAdmin):
    list_display = ['title', 'off_percent', 'subtitle', 'emoji', 'bg_color', 'is_active', 'order']
    list_editable = ['off_percent', 'is_active', 'order']
    list_filter = ['is_active']
    search_fields = ['title', 'subtitle']
    ordering = ['order', '-created_at']


@admin.register(CartOffer)
class CartOfferAdmin(admin.ModelAdmin):
    list_display = ['title', 'min_cart_value', 'reward_type', 'percent_off', 'flat_off', 'free_item', 'is_active', 'order']
